...
def _resume_hunt(context, session, chat_id, hunt_id, spec):
    ...
    # spec['adults'] 는 아직 루프로 넘기지 않는다. 인원 기능이 붙으면 여기서
    # Passenger 리스트로 되만들어 넘긴다 — 조건은 이미 실려 와 있다.
    if spec['kind'] == HUNT_TRAIN:
        coro = _train_hunt_loop(
            ...
            spec['option'], spec['interval'],
        )
    else:
        coro = _hunt_loop(
            context, session, chat_id, hunt_id, label,
            spec['dep'], spec['arr'], spec['d'], spec['t'], spec['interval'],
        )
...
async def _start_hunt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_hunts[hunt_id] = {
        'task': task,
        'label': label,
        # 인원은 아직 1명 고정이다. /reserve 가 인원을 받게 되면 그 값을 넘긴다.
...
async def _start_train_hunt(update, context, train_idx, option):
    ...
    chat_hunts[hunt_id] = {
        ...
        'label': label,
        # 인원은 아직 1명 고정이다. /reserve 가 인원을 받게 되면 그 값을 넘긴다.
        'spec': _hunt_spec(label, dep, arr, d, t, interval, target=target, option=option),
    }
    save_state(context)
